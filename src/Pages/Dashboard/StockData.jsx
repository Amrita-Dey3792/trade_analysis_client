...
const StockData = () => {
  ...
  const { data, isLoading, error, refetch } = useQuery({
    ...
    queryFn: async () => {
      const response = await fetch(`http://localhost:8000/stockdata/?page=${page}`);
      if (!response.ok) {
  ...
  if(isLoading) {
    ...
    if(error) {
      return <p className="mt-10 mx-auto text-red-500">Failed to load stock data. Please try again.</p>;
    }
